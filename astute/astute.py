...
def astute(thres, binSize):
  ...
  for line in sys.stdin:
    ...
    #File format: timestamp ip_src sport ip_dst dport ip_len ip_proto tcp_flags
    words = line.split()
    ...
    ## Anomaly detection and window sliding
    if ts > binStart+binSize:
      ...
      if previousBin:
        ...
        for i in range(6):
          ...
          if i < 4:
            ...
            allKeys.append(list(set(np.nonzero(previousBin[i][:,0])[0].tolist()+np.nonzero(currentBin[i][:,0])[0].tolist())))
            delta.append(np.array([currentBin[i][k]-previousBin[i][k] for k in allKeys[i]]))
          ...
          if volAno:  
            ...
            while anoFound == -1 and top < 100:        #no more than 100 features per anomaly
              for flowAgg in noAnomaly[volType]:
                ...
                while abs(volCount[flowAgg]) < abs(volAno) and inc < top:
                  ...
                  if not (flowAgg in [4,5] and allKeys[flowAgg][anoFlow] < 100):              # ignore only port 80 or 0 kind of anomaly
            ...
            for flow in anomalousFeature[anoFound]:
              ...
              if anoFound == 0:
                ...
                dstPort=str(flow)
              print(srcIP+":"+srcPort+" "+dstIP+":"+dstPort+";"+str(binStart)+";"+str(binStart+binSize)+"\t1")
              
            #if anoFound != -1: 
              #ano = admd.anomaly_t()
              #ano.set_description(str(K))
              #ano.add_slice(slice)
              #tsS = admd.timestamp_t(sec=binStart, usec=0)
              #tsE = admd.timestamp_t(sec=binStart+binSize, usec=9999999)
              #ano.set_from(tsS)
              #ano.set_to(tsE)
              #ano.set_type(str(anoFound))
              #root.add_anomaly(ano)
      ...
      currentBin = [defaultdict(constant_factory()),defaultdict(constant_factory()),defaultdict(constant_factory()),defaultdict(constant_factory()), np.zeros( (2**16,2) ), np.zeros( (2**16,2) )]
      binStart += binSize
...
if __name__ == "__main__":  
  ...
  if len(sys.argv) < 3:
    print("usage: "+sys.argv[0]+" threshold binSize")
    exit()

  threshold = float(sys.argv[1])
  binSize = float(sys.argv[2])

  astute(threshold, binSize)
